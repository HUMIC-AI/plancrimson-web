...
import { EMOJI_SCALES, Explanation } from './Graph';
...
export function HoveredCourseInfo() {
  const {
    graph, hoveredClassId: courseId, setExplanation, explanation,
  } = useGraphContext();
  ...
  const props = useMemo<InfoCardProps>(() => {
    if (explanation) {
      return getExplanationModalProps(explanation, setExplanation);
    }

    if (courseId) {
      if (!course) {
        // wait until course loads
  ...
  }, [course, courseId, explanation, graph, setExplanation]);

  return <InfoCard {...props} />;
}

function getExplanationModalProps(explanation: Explanation, setExplanation: (text: Explanation | null) => void): InfoCardProps {
  return {
    ...
    content: (
      <div className="space-y-2 px-6 pb-64 leading-relaxed">
        {explanation.text ? (
          <>
            <p>
              {explanation.text}
            ...
            <p className="text-blue-primary">
              This text was generated by a language model and may be biased or inaccurate.
            </p>
          </>
        ) : <LoadingBars />}
      </div>
...
export function GraphInstructions({ direction }: { direction: 'row' | 'column' }) {
  ...
  return (
    <div className={classNames(
      'flex px-6 pb-6 gap-4',
      direction === 'column' ? 'flex-col-reverse' : 'flex-row items-center',
    )}
    >
      {direction === 'column' && userId && (
        <div className="pb-64">
          ...
          <p className="mb-4">
            Load a schedule into the graph:
          </p>
      ...
      <ul className="list-inside list-disc space-y-2">
        ...
        <li className="text-blue-primary">
          Measures of similarity are generated by a language model and may reflect societal biases.
        </li>
      </ul>
    </div>
  );
