...
const GridHeading: React.FC<{ title: string; data: EvaluationStatistics | null }> = function ({ title, data }) {
  if (!data) return null;
...
const InstructorFeedback: React.FC<{ evaluation: Evaluation; }> = function ({ evaluation }) {
  ...
  const components = {
    Lectures: instructorQuestions['Gives effective lectures or presentations, if applicable'],
    Accessible: instructorQuestions['Is accessible outside of class (including after class, office hours, e-mail, etc.)'],
    Enthusiasm: instructorQuestions['Generates enthusiasm for the subject matter'] || null,
    Discussion: instructorQuestions['Facilitates discussion and encourages participation'],
...
const EvaluationComponent: React.FC<{ report: Evaluation | Evaluation[] }> = function ({ report }) {
  ...
  const hoursData = courseEvaluation['On average, how many hours per week did you spend on coursework outside of class? Enter a whole number between 0 and 168.'];
  ...
  const [overall, generalComponents] = useMemo(() => {
    ...
    const {
      ...
      'Assignments (exams, essays, problem sets, language homework, etc.)': Assignments,
      'Course materials (readings, audio-visual materials, textbooks, lab manuals, website, etc.)': Materials,
      'Feedback you received on work you produced in this course': Feedback,
      'Section component of the course': SectionData,
    } = general;
    return [overallEvl, {
      Assignments, Materials, Feedback, Section: SectionData,
    }];
  }, [general]);

  return (
    <Disclosure defaultOpen as="div" className="border-gray-800 border-2 rounded-lg">
      {({ open }) => (
        <>
          ...
          >
            <h3 className="font-bold">
              {`${courseEvaluation.year} ${courseEvaluation.season} — ${courseEvaluation.instructorName}`}
            </h3>
            <span className="flex items-center gap-4">
          ...
          <FadeTransition>
            <Disclosure.Panel className="space-y-4 p-4">
              <p>
                {courseEvaluation['Course Response Rate']?.invited || 'Unknown'}
                {' '}
                students total
              </p>
              ...
              {multipleInstructors
                ? report.map((evl) => <InstructorFeedback key={evl.instructorName} evaluation={evl} />)
